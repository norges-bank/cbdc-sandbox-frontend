...
import { DECIMAL_PLACEHOLDER, NOK_DECIMAL_PLACEHOLDER } from '../constants';
...
/**
 * Function that sets the number placeholder depending on locale
 *
 * @returns Either 0,0000 for NO or 0.0000 for EN/rest
 */
export const localePlaceholder = () => {
  const lang = getLang();
  if (lang === 'no') {
    return NOK_DECIMAL_PLACEHOLDER;
  } else return DECIMAL_PLACEHOLDER;
};
...
export const displayAsCurrency = (string, decimals = 4) => {
  return new Intl.NumberFormat(getLang(), { minimumFractionDigits: decimals }).format(
    insertDecimalSeparator(string, decimals)
  );
};
...
/**
 * Convenience function to convert BigNumber to Number
 *
 * @param {Bignumber} bn
 * @returns {Number}
 */
export const bigNumberToNumber = (bn) => {
