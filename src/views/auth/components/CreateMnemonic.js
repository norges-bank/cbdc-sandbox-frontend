      ...
      <Box>
        <Grid spacing={1} container>
          {phrases.map((phrase, index) => (
            <Grid key={index} item xs={6} md={3} sm={3}>
              <Box key={index} className="chip">
                <Typography sx={{ fontSize: '14px' }} variant="p">{`${
                  index + 1
                }. ${phrase}`}</Typography>
              </Box>
            </Grid>
      ...
      <Box>
        <Grid container spacing={3}>
          <Grid item xs={5} sm={5} md={5}>
            <Button sx={{ width: '100%' }} className="button-wallet" wide>
              BACK
            </Button>
          </Grid>
