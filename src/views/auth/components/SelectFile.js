    ...
    <Box>
      <CardContent sx={{}}>
        <Typography
          ...
          variant="p"
          sx={{ fontWeight: 'bold', fontSize: '15px', color: '#153443' }}>
          SELECT YOUR KEYSTORE FILE
        </Typography>
        <br />
        <Typography variant="p" sx={{ fontWeight: 'bold', fontSize: '13px', color: '#153443' }}>
          Select keystore file that unlocks your wallet
        </Typography>
        {error ? (
      ...
      <CardActions sx={{ p: 2 }}>
        <Grid container spacing={3}>
          <Grid item xs={4} sm={4} md={4}>
            <Button sx={{ width: '100%' }} className="keystore-button" wide onClick={onBack}>
              BACK
            </Button>
          </Grid>
          <Grid item xs={8} sm={8} md={8}>
            <Button
              sx={{ width: '100%' }}
              onClick={handleKeystoreFileClick}
